...
struct CoinManager {
    ...
    let apiKey = "1F57AD4F-4A46-496A-B67F-8351B2AB5046"
    
    let currencyArray = ["AUD", "BRL","CAD","CNY","EUR","GBP","HKD","IDR","ILS","INR","JPY","MXN","NOK","NZD","PLN","RON","RUB","SEK","SGD","USD","ZAR"]
    
    func getCoinPrice(for currency: String) {
        
        //Step 1. Create a URL (Use String concatenation to add the selected currency at the end of the baseURL along with the API key.)
        let urlString = "\(baseURL)/\(currency)?apikey=\(apiKey)"
        
        //Use optional binding to unwrap the URL that's created from the urlString
        if let url = URL(string: urlString) {
            
            //Step 2. Create a URLSession (Create a new URLSession object with default configuration.)
            let session = URLSession(configuration: .default)
            
            //Step 3. Give the session a task (Create a new data task for the URLSession)
        ...
        //Step 4. Start the task (Start task to fetch data from bitcoin average's servers.)
            task.resume()
        }
    }
